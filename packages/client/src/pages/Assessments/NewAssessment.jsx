...
export const NewAssessment = () => {
  ...
  const calculateScoreAndRisk = (data) => {
    ...
    const score = yesNoFields.reduce((sum, field) => sum + parseInt(data[field] || `0`, 10), 0); // go through and get sum of values from fields
  ...
  return <div className="border border-dark rounded p-3">
    <Form onSubmit={handleSubmit(onSubmit, (err) => console.error(err))}>
      ...
      <Form.Group controlId="Instrument" className="mb-3">
        <div className="bg-danger border border-dark rounded p-4">
          <h1 className="text-white">Cat Assessment Info</h1>
        </div>
        <Form.Control
          type="hidden"
          defaultValue={1}
